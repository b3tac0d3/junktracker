...
$actionUrl = (string) ($actionUrl ?? url('/expenses'));
$mode = (string) ($mode ?? 'create');
$pageHeading = (string) ($pageHeading ?? ($mode === 'edit' ? 'Edit General Expense' : 'Add General Expense'));
$cancelUrl = (string) ($cancelUrl ?? url('/expenses'));
$submitLabel = (string) ($submitLabel ?? ($mode === 'edit' ? 'Save Changes' : 'Save Expense'));
...
<div class="page-header d-flex flex-wrap align-items-end justify-content-between gap-2">
    <div>
        <h1><?= e($pageHeading) ?></h1>
        <p class="muted">Not tied to a job</p>
    </div>
    <div>
        <a class="btn btn-outline-secondary" href="<?= e($cancelUrl) ?>"><?= e($mode === 'edit' ? 'Back to Expense' : 'Back to Expenses') ?></a>
    </div>
</div>

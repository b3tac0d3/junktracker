...
final class Invoice
{
    ...
    public static function indexBillingGrouped(
        ...
        if ($limit > 0) {
            $flatRows = $pastDueOnly
                ? $buckets['past_due']
                : array_merge($buckets['past_due'], $buckets['unpaid'], $buckets['paid']);
            $pageRows = array_slice($flatRows, max(0, $offset), $limit);
            $pageBuckets = [
                'past_due' => [],
                'unpaid' => [],
                'paid' => [],
            ...
            foreach ($pageRows as $row) {
                ...
                $bucket = self::billingPaymentBucket($row);
                if (isset($pageBuckets[$bucket])) {
                    $pageBuckets[$bucket][] = $row;
                }
            }
            $buckets = $pageBuckets;
        }
    ...
    public static function indexBillingRecordsCount(
        int $businessId,
        string $search = '',
        string $status = '',
        string $dateFrom = '',
    ...
    public static function findForBusiness(int $businessId, int $invoiceId): ?array
    {
        ...
        $jobIdSql = SchemaInspector::hasColumn('invoices', 'job_id') ? 'i.job_id' : 'NULL';
        $quoteIdSql = SchemaInspector::hasColumn('invoices', 'quote_id') ? 'i.quote_id' : 'NULL';
